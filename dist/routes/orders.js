...
const EXTRA_PRICES = {
    1: { name: "⚡ Яаралтай", price: 20000 },
    2: { name: "🌙 Шөнө / Амралтын өдөр", price: 15000 },
    3: { name: "🗺️ Олон цэг", price: 10000 },
};
...
async function countOrdersForDateSlot(tx, slotId, date) {
...
router.get("/slots", async (req, res) => {
    ...
    if (!date)
        return res.status(400).json({ message: "date шаардлагатай" });
    try {
        ...
        const dayStart = new Date(date + "T00:00:00.000Z");
        const dayEnd = new Date(date + "T23:59:59.999Z");
        const counts = await Promise.all(slots.map((s) => prisma_1.default.order.count({
            where: { slotId: s.id, createdAt: { gte: dayStart, lte: dayEnd }, status: { notIn: ["CANCELED"] } },
        })));
        return res.json(slots.map((s, i) => ({
...
router.post("/", auth_1.auth, async (req, res) => {
    const { serviceTypeId, zoneId, address, volume, volumeUnit, slotId, date, addOnIds = [], lat, lng, customerName, customerPhone, notes, 
    // Frontend-ээс илгээдэг нэмэлт талбарууд
    pitStatus, pitType, district, unit, timeSlot, extras = [], serviceType: serviceTypeLabel,
    ...
    const userExists = await prisma_1.default.user.findUnique({ where: { id: req.user.id } });
    if (!userExists) {
    ...
    const normalizedUnit = String(volumeUnit ?? "TON").toUpperCase().includes("M3") ? "M3" : "TON";
    try {
        const created = await prisma_1.default.$transaction(async (tx) => {
            ...
            const addOnIdNums = (addOnIds || []).map(Number).filter((n) => !isNaN(n));
            ...
            return await tx.order.create({
                data: {
                    ...
                    addOns: {
                        create: addOnIdNums
                            .filter((id) => dbAddOnMap.has(id)) // зөвхөн DB-д байгааг л хадгална
                            .map((id) => ({ addOnId: id })),
                    },
                },
        ...
        if (msg === "ZONE_NOT_FOUND")
            return res.status(404).json({ message: "Бүс олдсонгүй" });
...
router.get("/:id", auth_1.auth, async (req, res) => {
    ...
    try {
        const order = await prisma_1.default.order.findUnique({ where: { id }, include: ORDER_INCLUDE });
        if (!order)
...
router.put("/:id/cancel", auth_1.auth, async (req, res) => {
    ...
    try {
        ...
        if (existing.status === "IN_PROGRESS")
            return res.status(400).json({ message: "Явагдаж буй захиалгыг цуцлах боломжгүй" });
        await prisma_1.default.order.update({ where: { id }, data: { status: "CANCELED" } });
