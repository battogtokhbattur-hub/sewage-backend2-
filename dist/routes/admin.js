...
function orderToEmailPayload(o) {
...
═══════════════════════════════════════════════════════ */
router.get("/orders", auth_1.auth, auth_1.adminOnly, async (_req, res) => {
    try {
        let orders;
        try {
...
router.get("/drivers", auth_1.auth, auth_1.adminOnly, async (_req, res) => {
    try {
        // Driver model байхгүй бол хоосон массив буцаах
        let drivers = [];
        try {
            drivers = await prisma_1.default.driver.findMany({
                where: { isActive: true },
                orderBy: { id: "asc" },
            });
        }
...
router.post("/drivers", auth_1.auth, auth_1.adminOnly, async (req, res) => {
...
router.put("/drivers/:id", auth_1.auth, auth_1.adminOnly, async (req, res) => {
...
router.delete("/drivers/:id", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
...
router.put("/orders/:id/confirm", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendOrderConfirmedEmail)(orderToEmailPayload(updated));
                }
            }
            catch (e) {
                console.error(`[Confirm] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id/assign", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    const { driverId } = req.body;
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    if (!driverId)
        return res.status(400).json({ message: "driverId шаардлагатай" });
    try {
        const existing = await prisma_1.default.order.findUnique({ where: { id } });
        if (!existing)
            return res.status(404).json({ message: "Захиалга олдсонгүй" });
        ...
        const driver = await prisma_1.default.driver.findUnique({ where: { id: Number(driverId) } });
        if (!driver)
            return res.status(404).json({ message: "Жолооч олдсонгүй" });
        if (!driver.isActive)
            return res.status(400).json({ message: "Жолооч идэвхгүй байна" });
        const updated = await prisma_1.default.order.update({
            where: { id },
            data: {
                status: "ASSIGNED",
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendOrderAssignedEmail)({
                ...
                console.error(`[Assign] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id/start", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendOrderInProgressEmail)(orderToEmailPayload(updated));
                }
            }
            catch (e) {
                console.error(`[Start] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id/finish-service", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendInvoicePaymentEmail)(orderToEmailPayload(updated));
                }
            }
            catch (e) {
                console.error(`[FinishService] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id/confirm-payment", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendOrderCompletedEmail)({
                        ...orderToEmailPayload(updated),
                ...
                console.error(`[ConfirmPayment] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id/cancel", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    ...
    try {
        ...
        if (existing.status === "DONE")
            return res.status(400).json({ message: "Дууссан захиалгыг цуцлах боломжгүй" });
        if (existing.status === "CANCELED")
            return res.status(400).json({ message: "Аль хэдийн цуцлагдсан" });
        const updated = await prisma_1.default.order.update({
        ...
        setImmediate(async () => {
            try {
                if (updated.user?.email) {
                    await (0, statusEmails_1.sendOrderCanceledEmail)({
                        ...orderToEmailPayload(updated),
                ...
                console.error(`[Cancel] email failed #${id}:`, e);
            }
        });
        res.json(updated);
    }
...
router.put("/orders/:id", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
...
router.delete("/orders/:id", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
        // OrderAddOn эхлээд устгана (FK constraint)
        await prisma_1.default.orderAddOn.deleteMany({ where: { orderId: id } });
...
═══════════════════════════════════════════════════════ */
router.get("/users", auth_1.auth, auth_1.adminOnly, async (_req, res) => {
    try {
        ...
        const result = await Promise.all(users.map(async (u) => {
            const activeOrdersCount = await prisma_1.default.order.count({
                where: {
                    userId: u.id,
...
═══════════════════════════════════════════════════════ */
router.get("/analytics", auth_1.auth, auth_1.adminOnly, async (_req, res) => {
    try {
        const now = new Date();
        ...
        const newUsersThisWeek = await prisma_1.default.user.count({
        ...
        const activeUsersList = await prisma_1.default.order.findMany({
            where: {
                status: { in: ["PENDING", "CONFIRMED", "ASSIGNED", "IN_PROGRESS", "AWAITING_PAYMENT"] },
            },
            select: { userId: true },
        ...
        const allUsersWithOrders = await prisma_1.default.order.findMany({
            select: { userId: true },
            distinct: ["userId"],
        });
        const usersWithOrdersIds = new Set(allUsersWithOrders.map((o) => o.userId));
        ...
        const ordersToday = await prisma_1.default.order.count({
            where: { createdAt: { gte: today } },
        });
        const ordersThisWeek = await prisma_1.default.order.count({
            where: { createdAt: { gte: weekAgo } },
        });
        const ordersThisMonth = await prisma_1.default.order.count({
        ...
        for (const s of statuses) {
            statusCounts[s] = await prisma_1.default.order.count({ where: { status: s } });
        }
        // 6) Орлого (DONE статуст байгаа захиалгуудын нийлбэр)
        const doneOrders = await prisma_1.default.order.findMany({
            where: { status: "DONE" },
            select: { priceTotal: true, createdAt: true },
        });
        const totalRevenue = doneOrders.reduce((sum, o) => sum + Number(o.priceTotal || 0), 0);
        const revenueThisMonth = doneOrders
            .filter((o) => o.createdAt >= monthAgo)
            .reduce((sum, o) => sum + Number(o.priceTotal || 0), 0);
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const c = await prisma_1.default.order.count({
        ...
        async function statsForPeriod(since) {
            ...
            const revenue = completedOrders.reduce((sum, o) => sum + Number(o.priceTotal || 0), 0);
            const distinctUsers = await prisma_1.default.order.findMany({
                where: { createdAt: { gte: since } },
                select: { userId: true },
                distinct: ["userId"],
            });
...
router.get("/users/:id", auth_1.auth, auth_1.adminOnly, async (req, res) => {
    const id = Number(req.params.id);
    if (isNaN(id))
        return res.status(400).json({ message: "Буруу ID" });
    try {
...
═══════════════════════════════════════════════════════ */
router.get("/report/csv", auth_1.auth, auth_1.adminOnly, async (req, res) => {
